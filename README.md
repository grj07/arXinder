...
In a linux virtual terminal navigate to the main directory which contains the executable file:
...
The software consists of a user interface (ui.c linked to pfuncs and gfuncs) which displays entries from a feed. This feed is determined by subject area and date.

The user interface is written in C using ncurses, and processes a feed of a given subject (as set in conf/subjects.conf) for a given day (starting with the most recent feed and running backwards in time).
...
mkchl.py -- generates html output
